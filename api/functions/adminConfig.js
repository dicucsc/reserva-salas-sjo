...
app.http('adminConfig', {
  ...
  handler: async (request, context) => {
    try {
      ...
      if (!cfg) {
        return { status: 400, jsonBody: { ok: false, error: 'Recurso inválido' } };
      }
      ...
      if (action === 'list') {
        ...
        return { jsonBody: { ok: true, data: items } };
      }
      ...
      if (action === 'save') {
        ...
        if (resource === 'equipos') {
          if (!data.Nombre) return { status: 400, jsonBody: { ok: false, error: 'Nombre requerido' } };
          ...
          if (!data.HoraInicio || !data.HoraFin) return { status: 400, jsonBody: { ok: false, error: 'Horas requeridas' } };
        ...
        return { jsonBody: { ok: true, data: { ID: id } } };
      }

      if (action === 'delete') {
        if (!data.ID) return { status: 400, jsonBody: { ok: false, error: 'ID requerido' } };
        await deleteEntity(cfg.table, cfg.pk, String(data.ID));
        return { jsonBody: { ok: true } };
      }

      return { status: 400, jsonBody: { ok: false, error: 'Acción inválida' } };
    } catch (err) {
      context.error('adminConfig error:', err);
